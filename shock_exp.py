...
import numpy as np
from ui.shock_exp_ui import ShockExpUI
from da.shock_exp_da import ShockExpDA
from et.shock_exp_et import ShockExpET

# ShockExp class provides high-level functions controlling the flow 
...
class ShockExp:    
    def __init__(self, subj_id=None, exp_type='high'):
        self.user_interface = ShockExpUI()  
        self.data_access = ShockExpDA(subj_id, exp_type)
        self.eye_tracker = ShockExpET(user_interface=self.user_interface, 
...
#        TODO: think about moving is_club_left = not is_club_left to run_block
        is_club_left = False
    ...
    def run_block(self, block_number, rewards, max_n_trials, is_club_left=True, is_baseline=False):
        ...
        while accumulated_points < threshold:             
            card_chosen, points_earned, response_dynamics_log, choice_info = \
    ...
    def run_trial(self, accumulated_points, threshold, block_no, trial_no, rewards,
        ...
        trial_info = {'exp_type': self.exp_info['exp_type'],
                      ...
                      'threshold': threshold,
                      'club_threat_probability': self.exp_info['club_threat_probability'],
                      'spade_threat_probability': self.exp_info['spade_threat_probability']}
        ...
        threat_delivered = self.user_interface.show_trial_end_screen(points_earned, 
                                                accumulated_points+points_earned, trial_info)

        choice_info = [self.exp_info['subj_id'], block_no, trial_no, is_club_left, card_chosen, 
                        points_earned, threat_delivered, rewards[0], rewards[1], 
                        response_time, self.exp_info['club_threat_probability'], 
                        self.exp_info['spade_threat_probability']]

#        # drift correction after every fifth trial
        if trial_no % 5 == 0:
            self.eye_tracker.correct_drift()        
        
        return card_chosen, points_earned, response_dynamics_log, choice_info
